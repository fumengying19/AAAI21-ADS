...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, alpha=0.05, num_proto=3):
    ...
    def forward(self, x):
        ...
        feat1 = self.conv5(out).view(out.size(0), -1)
        feat2 = self.conv6(out).view(out.size(0), -1)
        feat1 = self.classifier1(feat1)
        feat2 = self.classifier1(feat2)
        # score1 = self.classifier1(self.linear1(feat1))
        # score2 = self.classifier2(self.linear2(feat2))
        # out = out.view(out.size(0), -1)
        score1 = self._compute_prob1(feat1)
        score2 = self._compute_prob2(feat2)

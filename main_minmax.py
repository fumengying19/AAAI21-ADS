...
def get_uncertainty(umodels, uunlabeled_loader):
    ...
    with torch.no_grad():
        for (inputs, labels) in uunlabeled_loader:
            inputs = inputs.cuda()
            # labels = labels.cuda()
            ...
            # disc_score = torch.tensor(1) - torch.cosine_similarity(output1, output2)
            # pred_loss = torch.mul(ent, disc_score)

            # pred_loss = torch.tensor(1) - torch.cosine_similarity(output1, output2)
...
if __name__ == '__main__':
    ...
    for trial in range(cfg.ACTIVE_LEARNING.TRIALS):
        ...
        # start
        # labeled_set = list(np.load('./output/get_models/l_set.npy'))[:10000]
        # end
        ...
        # start
        # models['backbone'].load_state_dict(
        #         torch.load('./output/get_models/active_resnet18_cifar10_cycle' +
        #                    '9_epoch199' + '.pth')['state_dict_backbone'])
        # end
        ...
        for cycle in range(cfg.ACTIVE_LEARNING.CYCLES):
            ...
            cos_criterion = Mycosloss()
            criterion_re = Myrecosloss()
            ...
            # # Randomly sample 10000 unlabeled data points
            # random.shuffle(unlabeled_set)
            # subset = unlabeled_set[:cfg.ACTIVE_LEARNING.SUBSET*2]
            # # Create unlabeled dataloader for the unlabeled subset
            # unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=cfg.TRAIN.BATCH,
            #                               sampler=SubsetSequentialSampler(subset),
            #                               pin_memory=True)
            # uncertainty_coarse = get_uncertainty(models, unlabeled_loader)
            # arg_coarse = np.argsort(uncertainty_coarse)
            # budget_coarse = cfg.ACTIVE_LEARNING.SUBSET
            # selected_set = list(torch.tensor(subset)[arg_coarse][-budget_coarse:].numpy())
            # random.shuffle(selected_set)
            # unlabeled_loader = DataLoader(unlabeled_dataset, batch_size=cfg.TRAIN.BATCH,
            #                               sampler=SubsetSequentialSampler(selected_set),
            #                               pin_memory=True)

            # dataloaders, unlabeled_loader, unlabeled_set = update_dataloaders_single(
            #     cfg,
